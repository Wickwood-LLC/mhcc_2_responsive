...
$art_style = '';
$art_head  = '';
function mhcc_2_process_html(&$variables)
{
 ...
 * @ingroup themeable
 */
function mhcc_2_button($variables)
{
...
function mhcc_2_preprocess_page(&$vars){
    ...
    if (isset($vars['node']->panelizer['page_manager']->pnid) || page_manager_get_current_page()) {
 ...
 * @ingroup themeable
 */
function mhcc_2_menu_local_task($variables)
{
 ...
 */
function mhcc_2_feed_icon($variables)
{
 ...
 * @ingroup themeable
 */
function mhcc_2_node_preview($variables)
{
    $node = $variables['node'];
...
function get_terms_D7($content)
{
    ...
    foreach (array_keys($content) as $name) {
        ...
        if ($field_type == null || $field_type != "taxonomy_term_reference") {
            continue;
        }
 ...
 */
function mhcc_2_form_alter(&$form, &$form_state, $form_id)
{
...
function mhcc_2_form_element_label($variables)
{
...
function mhcc_2_form_element($variables)
{
    $element = &$variables['element'];
    // This is also used in the installer, pre-database setup.
    $t = get_t();
...
function mhcc_2_pager($variables)
{
    $tags       = $variables['tags'];
    $element    = $variables['element'];
